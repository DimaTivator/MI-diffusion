...
def estimate_MI_steps(model, A_data, B_data, dt=0.01, T=5, batch_size=100_000, device="cuda"):
    ...
    for i in trange(len(t_range) - 1):
        ...
        score_A = score_func(model, torch.cat([X_t, torch.zeros_like(B)], dim=1), t, torch.zeros(batch_size)).cpu()
